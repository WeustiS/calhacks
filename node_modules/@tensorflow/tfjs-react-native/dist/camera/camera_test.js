...
async function createGLContext() {
    return expo_gl_1.GLView.createContextAsync();
}
const expectArraysEqual = tfjs_core_1.test_util.expectArraysEqual;
let gl;
jasmine_util_1.describeWithFlags('toTexture', test_env_registry_1.RN_ENVS, () => {
    beforeAll(async () => {
        if (gl == null) {
    ...
    it('should not throw', async () => {
        ...
        const width = 2;
        const depth = 4;
        const inTensor = tf.truncatedNormal([height, width, depth], 127, 40, 'int32');
        let texture;
        expect(async () => {
            texture = await camera_1.toTexture(gl, inTensor);
        }).not.toThrow();
    ...
    it('should roundtrip succesfully', async () => {
        ...
        const depth = 4;
        const inTensor = tf.truncatedNormal([height, width, depth], 127, 40, 'int32');
        const texture = await camera_1.toTexture(gl, inTensor);
        const outTensor = camera_1.fromTexture(gl, texture, { width, height, depth }, { width, height, depth });
    ...
    it('throws if tensor is not int32 dtype', async () => {
        ...
        const width = 2;
        const depth = 4;
        const floatInput = tf.truncatedNormal([height, width, depth], 127, 40, 'float32');
        expectAsync(camera_1.toTexture(gl, floatInput)).toBeRejected();
    });
    it('throws if tensor is not a tensor3d dtype', async () => {
        ...
        const width = 2;
        const depth = 4;
        const oneDInput = tf.truncatedNormal([height], 127, 40, 'int32');
        //@ts-ignore
        expectAsync(camera_1.toTexture(gl, oneDInput)).toBeRejected();
        const twoDInput = tf.truncatedNormal([height, width], 127, 40, 'int32');
        //@ts-ignore
        expectAsync(camera_1.toTexture(gl, twoDInput)).toBeRejected();
        const fourDInput = tf.truncatedNormal([batch, height, width, depth], 127, 40, 'int32');
        //@ts-ignore
        expectAsync(camera_1.toTexture(gl, fourDInput)).toBeRejected();
    });
});
jasmine_util_1.describeWithFlags('fromTexture:nearestNeighbor', test_env_registry_1.RN_ENVS, () => {
    let texture;
    let input;
    const inShape = [4, 4, 4];
    beforeAll(async () => {
    ...
    beforeEach(async () => {
        texture = await camera_1.toTexture(gl, input);
    });
    afterAll(() => {
        tf.dispose(input);
    });
    it('same size alignCorners=false', async () => {
        const output = camera_1.fromTexture(gl, texture, {
            height: inShape[0],
            width: inShape[1],
            depth: inShape[2],
        }, {
    ...
    it('same size, should drop alpha channel', async () => {
        await camera_1.detectGLCapabilities(gl);
        const expected = tf.tensor3d([
            [
                [200, 201, 202],
                [190, 191, 192],
...
jasmine_util_1.describeWithFlags('fromTexture:bilinear', test_env_registry_1.RN_ENVS, () => {
    let texture;
    let input;
    const inShape = [4, 4, 4];
    beforeAll(async () => {
...
//# sourceMappingURL=camera_test.js.map